...
const User = ({item}) => {
    ...
    useEffect(() => {
      const fetchFriendRequests = async() =>{
        try{
          const response = await fetch (`http://192.168.30.231:8000/friend-requests/sent/${userId}`)
    ...
    console.log("Friends Requests sent:",friendRequests);

    useEffect(() => {
      const fetchUserFriends = async() => {
        try{
          const response = await fetch(`http://192.168.30.231:8000/friends/${userId}`);
          ...
          if(response.ok){
            ...
            console.log("Error ritrieving user friends",response.statusText);
          }
        }catch(err) {
          console.log("Error",err);
        }
  ...
  return (
    <Pressable style={{flexDirection:"row",alignItems:"center",marginVertical:10}}>
      ...
      ): requestSent || friendRequests.some((friends) => friendRequests._id === item._id) ? (
        <Pressable style={{
